...
void fifo_init(struct fifo_buffer *fifo)
{
    /* set head, tail, count to 0
       sema_init slots_free to FIFO_SIZE (all slots available)
       sema_init slots_used to 0 (nothing to read yet)
       mutex_init lock */
}

int fifo_write(struct fifo_buffer *fifo, const char *msg, size_t len)
{
    /* down_interruptible slots_free
         blocks here if buffer is full - this is the backpressure point
         returns -ERESTARTSYS if interrupted by signal
       mutex_lock
       copy msg into messages[tail]
       advance tail with wraparound: tail = (tail + 1) % FIFO_SIZE
       increment count
       mutex_unlock
       up slots_used to wake any blocked reader */
}
